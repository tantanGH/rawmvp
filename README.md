...
ディスク上の無圧縮動画データを逐次読み取りしながら指定した画面モード・fpsで再生します。
 ...
 - 実験的にテキストVRAMを使う 512x512x16色パレット, 768x512x16色パレットにも対応
...
常駐する際には以下のパラメータをつけることを推奨します。(音量固定、1chのみ、割り込み間隔調整、単音)
...
テキストVRAMを使う512t/768tモードの場合は、TVRAMに直接転送できる形式のデータを4プレーン分繋げたものを用意します。横512の場合はグラフィック256と同様、2フレーム分を横に繋げて横幅1024のデータとします。横768の場合はグラフィック384と同様、右にパディングして横幅1024のデータとします。また、RAW動画データファイル名の拡張子を.PALに変えたファイルを用意し、フレームごとの16色分のパレットデータを入れておく必要があります。このモードは単なる実験で将来的に削除するかも。
