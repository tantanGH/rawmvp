...
メモリ転送のオーバーヘッドが発生するため、高フレームレートでの再生にはMPUパワーが必要です(目安:68030/100MHz)。
...
(*1) ... PCM8PP.Xは上位の割り込みとの兼ね合いで再生を自動停止する場合がある。WindrvXMからGVRAMへのデータロードを行なった場合もこの条件に該当するため再生がすぐに停止してしまう。このため、RAWMVP.XはXM6g+PCM8PP.Xの組み合わせで再生しようとした時は自動的にメインメモリを経由してGVRAMにデータを読み込みます。16bitPCM音声は正しく再生されますが、オーバーヘッドが発生するため30fpsでの再生を行うにはMPUを68030-100MHz程度に設定する必要があります。
...
* 0.5.8 (2023/08/22) ... XM6g + Windrv + PCM8PP で16bitPCM音声が鳴らない問題の対策を行なった。-xの挙動を元に戻した。XEiJを自動認識してメモリ転送を自動有効にするようにした。
