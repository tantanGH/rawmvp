...
また、以下の4つのタグを使うと画面最下段および最上段に固定テキストを表示することができます。クレジット表記などに使うことを想定しています。

    CREDIT:最下段に表示するクレジット
    CREDIT2:最下段-1段に表示するクレジット
    CREDIT3:最上段+1段に表示するクレジット
    CREDIT4:最上段に表示するクレジット
...
## 再生方法
    ...
    usage: rawmvp [options] <screen-width> <view-height> <fps> <movie-file> <audio-file>
      ...
      view-height ... 1 - 256
          ...
          audio-file ... pcm file (.pcm|.p21|.p31|.s22|.s24|.s32|.s44|.s48|.zmd)
...
* 0.7.8 (2023/11/01) ... 外部同期16bitPCM再生の仕様を一部変更。
* 0.7.7 (2023/10/29) ... CRTMOD16無しで横384モード再生した後にドットクロックを戻していなかった問題を修正。
* 0.7.6 (2023/10/26) ... CRTMOD16無しでも横384モードで再生できるようにした。
